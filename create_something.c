...
*\details Ce fichier contient les trois fonctions de base de création de joueur : \a create_keyboard , \a create_client , \a create_server 
*/
...
*\details Cette fonction permet au main de créer un joueur de type \a keyboard , qui joue sur la machine où le programme est lancé
*\param t paramètre de type \a token (lui-même type \a enum) pouvant prendre 3 valeurs : \a NOTHING , \a RED ou \a YELLOW (couleur des jetons)
...
player create_ia(token t,depth d) {
	...
	player_ia->player_kind=IA ; /* On initialise le joueur de type player pour que tout corresponde à un type de jeu IA */
...
*\details Cette fonction permet au main de créer un joueur de type \a server , à l'aide des arguments passés dans la ligne de commande
*\param t paramètre de type \a token (lui-même type \a enum) pouvant prendre 3 valeurs : \a NOTHING , \a RED ou \a YELLOW (couleur des jetons)
...
*\details Cette fonction permet au main de créer un joueur de type \a client , grâce aux arguments passés en ligne de commande
*\param t paramètre de type \a token (lui-même type \a enum) pouvant prendre 3 valeurs : \a NOTHING , \a RED ou \a YELLOW (couleur des jetons)
