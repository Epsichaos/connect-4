...
token play(player p1, player p2) {
	if(p1->player_kind==KEYBOARD) {
	    ...
	    g=create_grid() ;
	    print_grid(g);
	    int compteur=0 ; /* Compteur qui s'incrémente à chaque tour de jeu, et qui sert à déterminer si la grille est pleine */
	    int token_winner=0 ; /* Servira à retourner le jeton du vainqueur */
